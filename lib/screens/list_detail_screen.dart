...
class ListDetailScreen extends StatelessWidget {
            ...
            onPressed: () async {
              if (formKey.currentState!.validate()) {
                try {
                  ...
                  print('Tentative d\'ajout de l\'article: ${newItem.name}, Liste ID: $listId');
  ...
  Future<void> _showEditItemDialog(BuildContext context, BuyItem item, String itemId) async {
            ...
            onPressed: () async {
              if (formKey.currentState!.validate()) {
                try {
                  ...
                  print('Tentative de modification de l\'article: $itemId, Liste ID: $listId');
                  await FirestoreService().updateItem(listId, itemId, updatedItem);
                  Navigator.pop(context);
                  ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _shareList(BuildContext context, List<BuyItem> items) async {
    ...
    final list = BuyList(
      ...
      expirationDate: listData['expirationDate'] != null
      ...
      pw.Page(
        build: (pw.Context context) => pw.Column(
          ...
          children: [
            ...
            pw.Text('Description : ${list.description}', style: pw.TextStyle(fontSize: 16)),
            ...
            pw.Table(
              ...
              children: [
                pw.TableRow(
                  decoration: pw.BoxDecoration(color: PdfColor(0.78, 0.78, 0.78)),
                  children: [
                    pw.Padding(
    ...
    final file = File('${directory.path}/liste_${list.name.replaceAll(' ', '_')}.pdf');
      ...
      body: StreamBuilder<List<BuyItem>>(
        ...
        builder: (context, snapshot) {
          ...
          if (snapshot.hasError) {
            print('StreamBuilder: Erreur lors du chargement des articles: ${snapshot.error}');
            return Center(child: Text('Erreur de chargement des articles: ${snapshot.error}'));
          }
          final items = snapshot.data ?? [];
          print('StreamBuilder: ${items.length} articles chargés pour listId: $listId');
          print('Articles: ${items.map((item) => "${item.name} (ID: ${item.firestoreId})").toList()}');
          if (items.isEmpty) {
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    print('Affichage de l\'article: ${item.name}, itemId: $itemId, listId: $listId');
                    return ListTile(
                      title: Text(item.name),
                      subtitle: Text('Prix: ${item.price.toStringAsFixed(2)} € | Qté: ${item.quantity.toStringAsFixed(0)}'),
                      trailing: Row(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          IconButton(
                            ...
                            onPressed: () {
                              if (itemId.isEmpty) {
                                print('Erreur: ID de l\'article invalide pour ${item.name}');
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(content: Text('Erreur: ID de l\'article invalide')),
                                );
                                return;
                              }
                            ...
                            onPressed: () async {
                              ...
                              final confirm = await showDialog<bool>(
                                ...
                                builder: (context) => AlertDialog(
                                  ...
                                  content: Text('Voulez-vous supprimer l\'article "${item.name}" ?'),
                                  actions: [
                                    TextButton(
                              ...
                              if (confirm == true) {
                                try {
                                  print('Lancement de la suppression de l\'article: $itemId, listId: $listId');
                                  await FirestoreService().deleteItem(listId, itemId);
                                  ScaffoldMessenger.of(context).showSnackBar(
